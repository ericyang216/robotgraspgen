...
def sample_gaussian(m, v):
    ...
    e = torch.randn_like(v, requires_grad=True)
    z = m + e * torch.sqrt(v)
...
def log_normal(x, m, v):
    ...
    # N = torch.distributions.multivariate_normal.MultivariateNormal(m, torch.diag_embed(v))
    # log_prob = N.log_prob(x)

    log_exp = torch.sum(-0.5 * torch.mul(m - x, m - x) / v, dim=-1)
    det = torch.prod(v, dim=-1) 
    scale = torch.sqrt(det * ((2 * np.pi) ** v.shape[-1]))
    log_prob = log_exp - torch.log(scale)
...
def log_normal_mixture(z, m, v):
    ...
    # in the batch
    ################################################################################
    # Compute log p for each Gaussian at z -> (batch, mix)
    densities = log_normal(z.unsqueeze(axis=-2), m, v)

    # Average log p over each Gaussian density (batch,)
    log_prob = log_mean_exp(densities, -1)
